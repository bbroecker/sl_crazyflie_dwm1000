...
class DWM1000BtCollvoid(CollvoidInterface):
    ...
    def calculate_velocity(self, current_target_velocity):
        ...
        reference_vel = copy.deepcopy(current_target_velocity)
        reference_vel = self.set_speed(reference_vel, speed)
        reference_vel = self.rotate_vel_by_speed(reference_vel, rotation_speed, dt)
        reference_vel = self.rotate_vel_by_angle(reference_vel, angle_increment)
