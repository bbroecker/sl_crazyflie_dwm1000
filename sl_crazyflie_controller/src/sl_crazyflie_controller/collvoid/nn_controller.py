...
class NNMovementWrapper:
    def __init__(self, cfg):
        ...
        with tf.Graph().as_default():
            ...
            with tf.variable_scope("MovementModel", reuse=None, initializer=initializer_movement) as scope:
                self.network_movement = DQN(num_actions=9, num_states=7, config=cfg, namespace="DQN",
                                            is_training=False,
    ...
    def predict_velocity(self,v_x, v_y, goal_dx, goal_dy, obstacle_dx, obstacle_dy, pose, delta_time):
        ...
        if np.random.rand(1) < self.cfg.epsilon:
            action = np.random.randint(0, self.cfg.num_actions)
            # actions[d_id] = 2
            # a = 5
        else:
    ...
    def calculate_velocity(self, current_target_velocity):
        ...
        if self.last_velocity is None or dt > 1.0 / self.update_rate:
            ...
            if len(self.current_obstacles) <= 0:
                ...
                obs_pose = self.get_closes_obs()
                ox = obs_pose.pose.position.x - self.current_pose.pose.position.x
                oy = obs_pose.pose.position.y - self.current_pose.pose.position.y
                ox, oy = self.rotate_vel(ox, oy, -get_yaw_from_msg(self.current_pose))

            vel_x = (self.current_pose.x - self.prev_pose.x) / dt
            vel_y = (self.current_pose.y - self.prev_pose.y) / dt
            vel_x, vel_y = self.rotate_vel(vel_x, vel_y, -get_yaw_from_msg(self.current_pose))

            self.last_velocity = self.nn.predict_velocity(vel_x, vel_y, gx, gy, ox, oy, dt)
            self.last_update = rospy.Time.now()
